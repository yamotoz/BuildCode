...
function calcHealthScore(data: RepoData): number {
  ...
  // Consistency — weeks with commits in last 12 (max 15)
...
function CompareBar({ label, valA, valB, nameA, nameB }: BarProps) {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: `radial-gradient(circle, ${getScoreColor(winnerScore)}15 0%, transparent 70%)`,
        }}
      />
      ...
      <div className="relative z-10 flex flex-col h-full gap-5">
        ...
        <div className="flex items-center justify-between">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider"
            style={{
              background: 'rgba(46, 116, 139, 0.1)',
              color: '#2E748B',
              border: '1px solid rgba(46, 116, 139, 0.2)',
            }}
          >
        ...
        <div className="space-y-3 flex-1">
          <CompareBar label="Popularidade" valA={dataA.stars} valB={dataB.stars} nameA={nameA} nameB={nameB} />
          <CompareBar label="Atividade" valA={dataA.commitsLast30Days} valB={dataB.commitsLast30Days} nameA={nameA} nameB={nameB} />
          <CompareBar label="Comunidade" valA={dataA.forks} valB={dataB.forks} nameA={nameA} nameB={nameB} />
        </div>

        {/* Quick facts */}
        <div className="grid grid-cols-2 gap-2 border-t border-[#2A3135] pt-4">
          {[
            ...
            <div key={i} className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-white/[0.02]">
              ...
              <div>
                <div className="text-[11px] font-semibold text-white leading-tight">{item.label}</div>
        ...
        >
          <span className="text-slate-400 font-medium">Vencedor geral</span>
